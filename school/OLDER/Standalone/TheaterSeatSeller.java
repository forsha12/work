...
public class TheaterSeatSeller {
    ...
    public TheaterSeatSeller() {
        ...
        for (int x = 0; x < NUM_ROWS; x++) //this goes through the whole array, counting up the seats in each price range
        {
          for (int y = 0; y < NUM_COLS; y++)
          {
            if (seats[x][y] == 10)
                pricesAvailable[0]++;
            else if (seats[x][y] == 20)
                pricesAvailable[1]++;
            else if (seats[x][y] == 30)
                pricesAvailable[2]++;
            else if (seats[x][y] == 40)
                pricesAvailable[3]++;
            else 
                pricesAvailable[4]++;
            }
    ...
    public void printSeats() {
        //printing out the seats available in the different ticket prices
        System.out.print("Tickets available by price: \n$10: " + pricesAvailable[0] + "\n$20: " + pricesAvailable[1] +
              "\n$30: " + pricesAvailable[2] + "\n$40: " + pricesAvailable[3] + "\n$50: " + pricesAvailable[4]);
        //blank line
        System.out.println();
        //print spacer
        System.out.print("     ");
    ...
    public boolean getByPrice(int price) {
        ...
        System.out.println("You chose to buy a ticket with price: $" + price);
            ...
            for (int x = 0; x < NUM_ROWS; x++) //goes through seats to find if there's any available
            {
              for (int y = 0; y < NUM_COLS; y++)
             {
                
                 if (seats[x][y] == price && retVal == false) //if the seat is the right price and the seat isn't already found
                 {
                     retVal = true;
                     seats[x][y] = 0; //making this seat unavailable
                     pricesAvailable[(price-1)/10]--; //decreases the amount of seats in that price range by one
                 }
                
            }
        }
        //does not change retVal if a seat isn't found
    ...
    public boolean getByLoc(int row, int col) {
        ...
        if (!(row >= NUM_ROWS || col >= NUM_COLS)) //only executes when given values are not out of range
        {
           if (seats[row][col] != 0) //now checks to see if seat is available
           {
               retVal = true; //shows that the seat was available
               pricesAvailable[((seats[row][col]-1)/10)]--; //decreases the priceAvailable value for the price range of the sold seat
               seats[row][col] = 0; //makes seat unavailable
           }
        }
